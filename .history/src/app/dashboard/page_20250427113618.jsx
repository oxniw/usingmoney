...
function page() {
    ...
    const add = async () => {
      if (prices && goalname !== "") {
        try {
          if (!isNaN(parseFloat(prices)) && !isNaN(parseFloat(goalname))) {
            ...
            setpriceandgoalname(res.data.data.goalname)
            setreport([]);
          } else {
    ...
    <div className="relative h-screen bg-black">
      ...
      <div className="w-full h-screen bg-white-200 relative">
        ...
        <div className="w-full h-xl flex justify-center items-center gap-10 p-4 ">
          <span className="font-bold w-full">goal name : <input type="text" name="goal name" id="goal name" className="rounded-xl text-black w-full" onChange={(e) => {setgoalname(e.target.value)}}/> </span>
          <span className="font-bold w-full">price : <input type="text" name="price" id="price" className="rounded-xl text-black w-full" onChange={(e) => {setprice(e.target.value)}}/></span>
