    ...
    <div className="relative h-screen bg-black">
      ...
      <div className="w-full bg-white-200 h-full relative gap-5 p-5 flex">{Object.entries(datas).map(([key, value]) => (
            <div key={key} className="">
              ...
              <ul>
                {value.map((item, idx) => (
                  <div className="bg-black-200 p-5 w-full h-full max-h-52 rounded-2xl bg-slate-950 text-white gap-10 p-4 relative transition-transform duration-300 hover:scale-110"
                  key={idx}
                  >
                    <div>{(item.name)}
                    </div>
