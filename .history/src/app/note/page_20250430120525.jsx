...
export default function NoteInput() {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      try {
        const response = await axios.post('https://backen-swart.vercel.app/get-notes', {
  ...
  const sendNoteToBackend = async (content) => {
    try {
      await axios.post('https://backen-swart.vercel.app/submit-note', {
        content,
        username: localStorage.getItem("username"),
        password: localStorage.getItem("password"),
      });
  ...
  const deleteNote = async (index) => {
    ...
    try {
      await axios.post('https://backen-swart.vercel.app/delete-note', {
        username: localStorage.getItem("username"),
        password: localStorage.getItem("password"),
  ...
  return (
    <div className="p-4">
      ...
      <button
        onClick={logout}
        className="mt-2 px-4 py-2 right-0 p-4 bg-red-500 text-white rounded transition-transform duration-300 hover:scale-110">
        back
      </button>
      <div className="mt-4">
