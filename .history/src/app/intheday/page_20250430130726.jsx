...
function page() {
    ...
    const movedata = async (index,move) => {
      const res = await axios.post("https://backen-swart.vercel.app/api/moveposition", {
    ...
    const dele = async (position) =>{
      const res = await axios.post("https://backen-swart.vercel.app/api/deletedata", {
        username: localStorage.getItem("username"),
        password: localStorage.getItem("password"),
        year:localStorage.getItem("years"),
        mounth: localStorage.getItem("mounth"),
    ...
    function check() {
      ...
      const getdata = async (year,mounth,day) => {
        ...
        const res1 = await axios.post("https://backen-swart.vercel.app/api/getcost", {
          username: localStorage.getItem("username"),
          password: localStorage.getItem("password"),
          year: localStorage.getItem("years"),
          mounth: localStorage.getItem("mounth"),
        ...
        if (income && expense && name !== "") {
          try {
            if (!isNaN(parseFloat(expense)) && !isNaN(parseFloat(income))) {
              const res = await axios.post("https://backen-swart.vercel.app/api/updatedata", {
                username: localStorage.getItem("username"),
                password: localStorage.getItem("password"),
    ...
    <div className="relative h-screen bg-black">
      ...
      <div className="w-full bg-white-200 h-full relative gap-5 p-5">
        ...
        <div className="w-full gap-2 flex p-5 relative justify-center items-center overflow-auto">
          ...
          <div className="w-full flex bg-slate-500 justify-top items-top rounded-xl">
            <h1 className="drop-shadow-[0_0_2px_black] text-2xl font-bold bg-white-100 from-cyan-500 to-blue-500 text-transparent bg-clip-text gap-12 p-2">balance percent: {(totalexpense === 0) ? 0 : (((totalincome - totalexpense)/totalincome)*100).toFixed(3)} %</h1>
          </div>

          <div className="w-full flex bg-slate-500 justify-top items-top rounded-xl">
            <h1 className="drop-shadow-[0_0_2px_black] text-2xl font-bold bg-white-100 from-cyan-500 to-blue-500 text-transparent bg-clip-text gap-12 p-2">expense percent: {(totalincome === 0) ? 0 : (100*totalexpense/totalincome).toFixed(3)} %</h1>
          </div>
